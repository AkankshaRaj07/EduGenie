...
export const downloadLibraryItem = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const material = await Library.findById(id);
    if (!material) {
      res.status(404).send('Library material not found.');
      return;
    }
    ...
    // Fallback if Render wiped the file from its ephemeral disk
    res.status(404).send(`
      <div style="font-family: sans-serif; text-align: center; margin-top: 50px;">
        <h2 style="color: #E05058;">File No Longer Available</h2>
        <p style="color: #64748b;">This file was removed from the server due to ephemeral storage reset on Render.</p>
        <p style="color: #64748b;">Since library files cannot be automatically regenerated by AI, please delete this entry from your library and re-upload the document.</p>
      </div>
    `);
  } catch (error: any) {
    res.status(500).send('Failed to download library material');
  }
};
